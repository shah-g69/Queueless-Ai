...
/**
 * Generate 1-Click Direct Gmail Compose URL (100% Guaranteed Delivery)
 */
export function getGmailComposeUrl({ email, service, plan }) {
  const subject = encodeURIComponent(`QueueLess AI Official Visit Pass - ${service || "NADRA Smart CNIC"}`);
  
  const checklistText = (plan?.documents_checklist || [])
    .map((d, i) => `${i + 1}. ${d.item} [${d.status}]`)
    .join("\n");
  ...
  const body = encodeURIComponent(
    `QueueLess AI — Official Citizen Visit Pass\n` +
    `========================================\n\n` +
    `Target Service: ${service || "NADRA Smart CNIC"}\n` +
    `Readiness Score: ${plan?.readiness_score || "75%"}\n\n` +
    `📋 MANDATORY DOCUMENTS CHECKLIST:\n${checklistText}\n\n` +
    `🗺️ COUNTER-BY-COUNTER ROADMAP:\n${stepsText}\n\n` +
    `💵 Official Fee: ${plan?.estimated_fee || "Standard Fee"}\n` +
    `💡 Pro-Tip: ${plan?.pro_tip || "Visit during low-rush evening hours."}\n\n` +
    `========================================\n` +
    `Generated by QueueLess AI & Fastn Multi-Agent Platform`
  );
